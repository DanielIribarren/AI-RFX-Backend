...
# 🌍 SISTEMA MULTI-AMBIENTE INTEGRADO
sys.path.append('.')
...
# Cargar .env.development si MIGRATION_ENV=development
if os.getenv('MIGRATION_ENV') == 'development':
    env_file = Path('.env.development')
    if env_file.exists():
        load_dotenv(env_file)
        print(f"🔧 Variables de entorno cargadas: {env_file}")
    else:
        load_dotenv('.env')
        print("🔧 Fallback a .env (desarrollo)")
else:
    load_dotenv('.env')
    print("🔧 Variables de entorno cargadas: .env")

# Ahora importar config con variables ya cargadas
from backend.core.config import (
    config, get_database_client, print_environment_info, 
    is_development, is_production, get_environment
)
...
class Day1Migration:
    ...
    🌍 MULTI-AMBIENTE: Soporta desarrollo y producción
    """
    ...
    def __init__(self):
        ...
        self.migration_branch = "saas-app"
    ...
    def _setup_environment_config(self):
        """Configuración específica por ambiente"""
        logger.info(f"🌍 Configurando migración para ambiente: {self.environment}")
        
        if self.is_development:
            logger.info("🧪 MODO DESARROLLO: BD de desarrollo (Buddy AI)")
            logger.info("🛡️ Producción 100% segura - no se toca")
            self.backup_prefix = "dev"
            self.safe_mode = True
        else:
            logger.info("🚀 MODO PRODUCCIÓN: BD de producción") 
            logger.info("⚠️ MIGRACIÓN FINAL - cuidado!")
            self.backup_prefix = "prod"
    ...
    def run_complete_day_1(self):
        ...
        try:
            ...
            if self.is_development:
                logger.info("🛡️ MODO SEGURO: Desarrollo - Producción intacta")
            else:
                logger.info("⚠️ MODO PRODUCCIÓN: Migración final")
            logger.info("=" * 60)
    ...
    def _validate_environment(self):
        ...
        logger.info(f"🔍 Validando entorno de {self.environment}...")
        
        # Verificar archivos base del proyecto
        required_files = [
            "backend/app.py",
            "backend/models",
            "backend/services",
        ...
        # Verificar archivos de configuración según ambiente
        if self.is_development:
        ...
        required_files.extend(env_files)
    ...
    def _validate_database_connection(self):
        """Validar conexión a base de datos según ambiente"""
        try:
            logger.info(f"🗄️ Validando conexión a BD ({self.environment})...")
            client = get_database_client()
            
            # Test de conexión simple
            result = client.table('companies').select('count', count='exact').execute()
            
            if self.is_development:
                logger.info(f"✅ Conectado a BD desarrollo - Tabla companies: {result.count} registros")
            else:
                logger.info(f"✅ Conectado a BD producción - Tabla companies: {result.count} registros")
                
            self.db_client = client
            
        except Exception as e:
            raise RuntimeError(f"Error conectando a BD ({self.environment}): {str(e)}")
    ...
    def _backup_database(self):
        ...
        logger.info(f"🗄️ Creando backup de base de datos ({self.environment})...")
        
        # 📊 BACKUP REAL: Obtener datos actuales de la BD
        backup_data = self._get_current_database_state()
        
        # Crear script de backup completo
        backup_script = f"""
-- BACKUP DE BASE DE DATOS RFX V2.2 → SAAS GENERAL
-- Fecha: {datetime.now().isoformat()}
-- Ambiente: {self.environment.upper()}
-- Pre-migración Día 1
...
-- 1. Conectar a Supabase Dashboard del ambiente apropiado
-- 2. SQL Editor → Paste este contenido
-- 3. Ejecutar para restaurar estado original

-- NOTA: Este es un backup real con datos actuales
-- Ambiente: {'Desarrollo (Buddy AI)' if self.is_development else 'Producción'}
"""
        
        backup_sql_path = self.backup_dir / f"{self.backup_prefix}_database_backup_{self.migration_date}.sql"
        ...
        backup_data_path = self.backup_dir / f"{self.backup_prefix}_data_backup_queries_{self.migration_date}.sql"
        with open(backup_data_path, 'w') as f:
            f.write(backup_data_script)
        
        logger.info(f"✅ Backup de BD {self.environment} creado:")
        logger.info(f"   📄 {backup_sql_path}")
        logger.info(f"   📄 {backup_data_path}")
        
        if self.is_development:
            logger.info("🛡️ BD de desarrollo respaldada - Producción segura")
        else:
            logger.info("⚠️ BD de producción respaldada - Migración crítica")
    
    def _get_current_database_state(self) -> str:
        ...
        try:
            ...
            main_tables = [
                'companies', 'requesters', 'suppliers', 'product_catalog',
                'rfx_v2', 'rfx_products', 'generated_documents', 'rfx_history',
                'supplier_evaluations', 'rfx_pricing_configurations',
                'coordination_configurations', 'cost_per_person_configurations',
                'tax_configurations', 'pricing_configuration_history'
            ]
            
            for table in main_tables:
                try:
                    result = self.db_client.table(table).select('count', count='exact').execute()
                    tables_info.append(f"-- {table}: {result.count} registros")
                except Exception as e:
                    tables_info.append(f"-- {table}: Error - {str(e)}")
            
            return f"""
-- ESTADO ACTUAL DE TABLAS ({self.environment.upper()}):
{chr(10).join(tables_info)}
-- Total de tablas verificadas: {len(main_tables)}
-- Fecha de backup: {datetime.now().isoformat()}
"""
        
        except Exception as e:
            logger.warning(f"⚠️ No se pudo obtener estado completo de BD: {str(e)}")
            return f"-- Estado de BD no disponible: {str(e)}"
...
"""
    ...
    def _create_migration_branch(self):
        ...
        try:
            ...
            # Usar branch de migración existente o crear uno nuevo
            try:
                # Intentar cambiar a branch existente
                subprocess.run([
                    "git", "checkout", self.migration_branch
                ], check=True, capture_output=True)
                logger.info(f"✅ Branch existente activado: {self.migration_branch}")
            except subprocess.CalledProcessError:
                # Si no existe, crear nuevo branch
                subprocess.run([
                    "git", "checkout", "-b", self.migration_branch
                ], check=True)
                logger.info(f"✅ Branch nuevo creado y activado: {self.migration_branch}")
...
{'=' * 60}
🌍 AMBIENTE: {self.environment.upper()}
{'🛡️ DESARROLLO - Producción segura' if self.is_development else '⚠️ PRODUCCIÓN - Migración final'}
...
✅ COMPLETADO EXITOSAMENTE:
   ...
   🗄️ Conexión a BD {self.environment} verificada
...
🚀 SIGUIENTE PASO: DÍA 2
   ...
   $ {'MIGRATION_ENV=development python day_2_migration.py' if self.is_development else 'MIGRATION_ENV=production python day_2_migration.py'}
...
def main():
    ...
    🌍 MULTI-AMBIENTE: Usa MIGRATION_ENV para determinar ambiente
    """
    try:
        migration = Day1Migration()
        
        # Mostrar información del ambiente antes de empezar
        print(f"\n🌍 MIGRACIÓN DÍA 1 - AMBIENTE: {migration.environment.upper()}")
        if migration.is_development:
            print("🛡️ MODO DESARROLLO: BD Buddy AI - Producción 100% segura")
        else:
            print("⚠️ MODO PRODUCCIÓN: BD de producción - ¡CUIDADO!")
        
        migration.run_complete_day_1()
        
        print(f"\n🎉 ¡DÍA 1 COMPLETADO EXITOSAMENTE EN {migration.environment.upper()}!")
        print("🚀 Para continuar con Día 2:")
        if migration.is_development:
            print("   MIGRATION_ENV=development python day_2_migration.py")
        else:
            print("   MIGRATION_ENV=production python day_2_migration.py")
